...
👉 orenbenn.github.io/albumranking
...
￼ Algorithme ELO : Les scores commencent tous à 1000 points. À chaque duel, le calcul de la probabilité de victoire ajuste les scores avec un facteur ￼.
...
￼ ELO Algorithm: Song ratings start at 1000 points. Every matchup outcome calculates expected victory probabilities and updates ratings using a dynamic ￼-factor of 32.
...
בעזרת שימוש בשיטת מד הכושר ELO (אותו אלגוריתם המשמש לדירוג שחקני שחמט), כל בחירה שלכם מעדכנת באופן דינמי את הניקוד של השירים. בסיום התהליך, תקבלו דירוג מדויק להפליא של השירים האהובים עליכם, יחד עם ניתוח סטטיסטי אישי.
✨ תכונות עיקריות
...
￼ מיון קטלוג ומניעת כפילויות: כדי למנוע עומס ויזואלי, האלגוריתם מנרמל את שמות האלבומים ומבצע בדיקות דמיון קפדניות על מנת להפריד בין אלבומי מקור למהדורות דלוקס תוך איחוד סינגלים.
￼ אלגוריתם דירוג ELO: הדירוג של כל השירים מתחיל ב-1000 נקודות. לאחר כל דו-קרב, סיכויי הניצחון מחושבים מחדש והניקוד מתעדכן דינמית עם מקדם שינוי של ￼.
